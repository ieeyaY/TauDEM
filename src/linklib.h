/*  linklib functions for streamnet to compute stream networks based on d8
  directions.
...
void setLinkInfo(long **LinkIdU1U2DMagShapeid, double **LinkElevUElevDLength,
                 double **PointXY, float **PointElevArea, tdpartition *elev,
                 tiffIO *elevIO);
void getNumLinksAndPoints(long &myNumLinks, long &myNumPoints);
void SendAndReciveLinks(int nx, int ny, tdpartition *idGrid,
                        tdpartition *contribs, tdpartition *flowDir,
                        tdpartition *src);
long getMagnitude(int32_t Id);
void appendPoint(int32_t Id, point *addPoint);
void setDownLinkId(int32_t Id, long dId);
streamlink *
createLink(int32_t u1, int32_t u2, long d,
           point *coord); //,long numCoords);  //, float dx, float dy);
...
// assignment link* takeOut(linkId){}  link* joinLinks(LinkId1,LinkId2){}/ending
// point of one link is the starting point of the other link. (same x and y or
// MPI abort)   movelink(Id)from one prosses to annother prosses/send and
// recive? Takes a link ID as input.  Finds the link in the linked list linkSet
// and removes the llnode from the linked list.  It returns a pointer to the
// link.  If the ID is not found it returns NULL
streamlink *takeOut(int32_t Id) {
  ...
  if (linkSet.head->data->Id == Id) {
    ...
    while (current->data->Id != Id &&
           current->next != NULL) { // search the linked list
      previous = current;
...
//	else
//		while(current != NULL){
//			if(current->data->coord[0].x == x &&
//current->data->coord[0].y == y){ 				ID = current->data->Id; 				break;//I found it.
//			}
//			current = current->next;
//		}
//	return ID;
...
bool sendLink(int32_t Id, int dest) {
  ...
  MPI_Type_get_extent(MPI_LONG, NULL, &extent);
  offsets[1] = 2 * extent;
  oldtypes[1] = MPI_FLOAT;
  blockcounts[1] = 3;
  ...
  int bsize = toSend->numCoords * sizeof(point) * 2 +
              MPI_BSEND_OVERHEAD; // Experimentally this seems to need to have
                                  // >47 added for overhead
  buf = new point[bsize];
...
long findLinkThatEndsAt(long x, long y, tdpartition *elev) {
  ...
  if (linkSet.numLinks == 0)
    ...
    while (current != NULL) {
      //			cout << "x:" <<
      //current->data->coord[current->data->numCoords - 1].x << endl;
      //          cout << "x:" << current->data->coord[current->data->numCoords
      //          - 1].x << endl;
      if (current->data->coord.back().x == gx &&
          current->data->coord.back().y == gy && !current->data->terminated) {
        ID = current->data->Id;
        break; // I found it.
      }
      current = current->next;
...
streamlink *
createLink(int32_t u1, int32_t u2, long d,
           point *coord) //,long numCoords) //, double dx, double dy)
{
  int size;
  MPI_Comm_size(MCW, &size);
  int rank;
  ...
  //	if(newLink->coord[i].x != newLink->coord[i+1].x && newLink->coord[i].y
  //!= newLink->coord[i+1].y) 		newLink->length += diag; 	else
  //if(newLink->coord[i].x == newLink->coord[i+1].x ) 		newLink->length += dy;
  //	else
  //		newLink->length += dx;
  //}
  newLink->shapeId = -1;
...
void SendAndReciveLinks(int nx, int ny, tdpartition *idGrid,
  ...
  if (rank % 2 == 0) { // even send
    ...
    if (rank != 0) {
      for (i = 0; i < nx; i++) { // count links to send down
        if (!contribs->isNodata(i, -1) &&
            contribs->getData(i, -1, tempShort) <
                0) { // i am on the boarder and I am negitive, check who points
                     // to me and send there id.
          int p = flowDir->getData((long)i - 1, (long)0, tempShort);
          if (i > 0 && flowDir->getData((long)i - 1, (long)0, tempShort) == 2 &&
              src->getData(i - 1, 0, tempShort) == 1 &&
              idGrid->getData(i - 1, 0, tempLong) >= 0) {
            toSend++;
          } else if (flowDir->getData((long)i, (long)0, tempShort) == 3 &&
                     src->getData(i, 0, tempShort) == 1 &&
                     idGrid->getData(i, 0, tempLong) >= 0) {
            toSend++;
          } else if (i < nx &&
                     flowDir->getData((long)i + 1, (long)0, tempShort) == 4 &&
                     src->getData(i + 1, 0, tempShort) == 1 &&
                     idGrid->getData(i + 1, 0, tempLong) >= 0) {
            toSend++;
          }
      ...
      MPI_Send(&toSend, 1, MPI_INT, rank - 1, rank,
               MCW);             // tell the reciver how many links to recive.
      for (i = 0; i < nx; i++) { // count links to send up
        if (!contribs->isNodata(i, -1) &&
          ...
          if (i > 0 && flowDir->getData((long)i - 1, (long)0, tempShort) == 2 &&
              src->getData(i - 1, 0, tempShort) == 1 &&
              idGrid->getData(i - 1, 0, tempLong) >= 0) {
            if (sendLink(idGrid->getData(i - 1, 0, tempLong),
              ...
              MPI_Abort(MCW, 2);
                     ...
                     src->getData(i, 0, tempShort) == 1 &&
                     idGrid->getData(i, 0, tempLong) >= 0) {
            if (sendLink(idGrid->getData(i, 0, tempLong),
                         rank - 1)) { // check to see if sent properly
              linksSent++;
            } else { // didn't send right.
              MPI_Abort(MCW, 2);
    ...
    if (rank != size - 1) {
      for (i = 0; i < nx; i++) { // count links to send down
        if (!contribs->isNodata(i, ny) &&
            contribs->getData(i, ny, tempShort) <
                0) { // i am on the boarder and I am negitive, check who points
                     // to me and send there id.
          int p = flowDir->getData((long)i - 1, (long)ny - 1, tempShort);
          if (i > 0 &&
              flowDir->getData((long)i - 1, (long)ny - 1, tempShort) == 8 &&
              src->getData(i - 1, ny - 1, tempShort) == 1 &&
              idGrid->getData(i - 1, ny - 1, tempLong) >= 0) {
            toSend++;
          } else if (flowDir->getData((long)i, (long)ny - 1, tempShort) == 7 &&
                     src->getData(i, ny - 1, tempShort) == 1 &&
                     idGrid->getData(i, ny - 1, tempLong) >= 0) {
            toSend++;
          } else if (i < nx &&
                     flowDir->getData((long)i + 1, (long)ny - 1, tempShort) ==
                         6 &&
                     src->getData(i + 1, ny - 1, tempShort) == 1 &&
                     idGrid->getData(i + 1, ny - 1, tempLong) >= 0) {
            toSend++;
          ...
          if (i > 0 &&
            ...
            if (sendLink(idGrid->getData(i - 1, ny - 1, tempLong),
              ...
              MPI_Abort(MCW, 2);
                     ...
                     src->getData(i, ny - 1, tempShort) == 1 &&
                     idGrid->getData(i, ny - 1, tempLong) >= 0) {
            if (sendLink(idGrid->getData(i, ny - 1, tempLong),
                         rank + 1)) { // check to see if sent properly
              linksSent++;
            } else { // didn't send right.
              MPI_Abort(MCW, 2);
    ...
    if (rank != 0) { // recv from bellow
      MPI_Recv(&toRecv, 1, MPI_INT, rank - 1, rank - 1, MCW,
               &stat); // how many do I recive?
      int i = 0;
      for (i = 0; i < toRecv; i++) { // recive links
        // MPI_Recv(&sent,1,MPI_INT,rank-1,rank-1,MCW,&stat);//how many do I
        // recive?
  ...
  if (rank % 2 == 1) { // odds send and evens recive
    ...
    for (i = 0; i < nx; i++) { // count links to send down
      if (!contribs->isNodata(i, -1) &&
          contribs->getData(i, -1, tempShort) <
              0) { // i am on the boarder and I am negitive, check who points to
                   // me and send there id.
        int p = flowDir->getData((long)i - 1, (long)0, tempShort);
        if (i > 0 && flowDir->getData((long)i - 1, (long)0, tempShort) == 2 &&
            src->getData(i - 1, 0, tempShort) == 1 &&
            idGrid->getData(i - 1, 0, tempLong) >= 0) {
        ...
        if (flowDir->getData((long)i, (long)0, tempShort) == 3 &&
            src->getData(i, 0, tempShort) == 1 &&
            idGrid->getData(i, 0, tempLong) >= 0) {
          toSend++;
        }
...
void setLinkInfo(long **LinkIdU1U2DMagShapeid, double **LinkElevUElevDLength,
  ...
  if (linkSet.numLinks == 0)
    ...
    while (current != NULL) {
      ...
      LinkIdU1U2DMagShapeid[counter][2] =
          begcoord + current->data->numCoords - 1;
      ...
      /*
                              LinkElevUElevDLength[counter][0] =
         current->data->elevU; LinkElevUElevDLength[counter][1] =
         current->data->elevD; LinkElevUElevDLength[counter][2] =
         current->data->length;
              */
      long i = 0;
      double cellarea = (elev->getdxA()) * (elev->getdyA());
      for (i = 0; i < current->data->numCoords; i++) {
        // elevIO->globalXYToGeo(current->data->coord[i].x,
        // current->data->coord[i].y,PointXY[pointsSoFar][0],PointXY[pointsSoFar][1]);
        ...
        // PointElevArea[pointsSoFar][2] =
        // current->data->coord[i].area*cellarea;
        PointElevArea[pointsSoFar][2] =
            current->data->coord.front().area * cellarea;
        //  Pop off queue
...
point *initPoint(tdpartition *elev, tdpartition *areaD8, tdpartition *lengths,
                 long i, long j) {
  point *thePoint;
...
bool ReceiveWaitingLinks(int rank, int size) {
  //  This returns the number of links waiting to receive from processor with
  //  input rank
  if (size <= 1)
    return (false); // If single processor nothing to do
  ...
  //  logic to see if terminating messages have been received from adjacent
  //  processes in which case the function returns false Tag 66 is used to pass
  //  a terminating message to adjacent processes indicating all links have been
  //  sent
  if (rank == 0) {
